...
public class FileUtils
{
	public static List<File> asList( File file ) throws IOException
	{
		...
		} else
		{
			if ( file.isDirectory() )
			{
				files = listFilesRecursively( file );
			}
		}
		return files;
	}
	...
	public static void copyFiles( List<File> filesToCopy, File inputDirectory, File outputDirectory ) throws IOException
	{
		...
		{
			File destinationFile = new File( outputDirectory.getAbsolutePath() + fileToCopy	.getAbsolutePath()
																							.replaceFirst( inputDirectory.getAbsolutePath(), "" ) );
